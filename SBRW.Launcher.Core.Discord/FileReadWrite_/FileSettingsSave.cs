...
using SBRW.Launcher.Core.Extras.Launcher_;
...
namespace SBRW.Launcher.Core.Extras.FileReadWrite_
{
    public class Save_Settings
    {
        ...
        {
            SettingFile = new Ini_File(Cache.Launcher_Settings);
            ...
            if (Launcher_Value.System_Unix && !SettingFile.KeyExists("InstallationDirectory"))
            {
            ...
            if (Launcher_Value.System_Unix && SettingFile.KeyExists("OldInstallationDirectory"))
            {
                SettingFile.DeleteKey("OldInstallationDirectory");
            }
            else if (!Launcher_Value.System_Unix && !SettingFile.KeyExists("OldInstallationDirectory"))
            {
            ...
            if (Launcher_Value.System_Unix && SettingFile.KeyExists("FilePermission"))
            {
                SettingFile.DeleteKey("FilePermission");
            }
            else if (!Launcher_Value.System_Unix && (!SettingFile.KeyExists("FilePermission") || string.IsNullOrWhiteSpace(SettingFile.Read("FilePermission"))))
            {
            ...
            if (!Launcher_Value.System_Unix)
            {
                ...
                if (Product_Version.GetWindowsNumber() == 6.1 && !SettingFile.KeyExists("PatchesApplied"))
                {
                    SettingFile.Write("PatchesApplied", Live_Data.Win_7_Patches);
                }
                else if (Product_Version.GetWindowsNumber() == 6.1 && SettingFile.KeyExists("PatchesApplied"))
                {
                    Live_Data.Win_7_Patches = SettingFile.Read("PatchesApplied");
                }
                else if ((Launcher_Value.System_Unix || Product_Version.GetWindowsNumber() != 6.1) && SettingFile.KeyExists("PatchesApplied"))
                {
            ...
            if (!UsingCustomProxyPort)
            {
                ...
                if (isNumeric)
                {
                    Proxy_Server.ProxyPort = new Random().Next(2017, Port);
                }
                else
                {
                    Proxy_Server.ProxyPort = new Random().Next(2017, 2021);
                }

                Log.Info("SETTINGS FILE: Random Generated Default Port -> " + Proxy_Server.ProxyPort);
            }
        ...
        public static void SaveSettings()
        {
            ...
            if (!Launcher_Value.System_Unix && SettingFile.Read("OldInstallationDirectory") != Live_Data.Game_Path_Old)
            {
                SettingFile.Write("OldInstallationDirectory", Live_Data.Game_Path_Old);
            }
            ...
            if (!Launcher_Value.System_Unix)
            {
                ...
                if ((SettingFile.Read("PatchesApplied") != Live_Data.Win_7_Patches) && Product_Version.GetWindowsNumber() == 6.1)
                {
                    SettingFile.Write("PatchesApplied", Live_Data.Win_7_Patches);
                }
            }
