...
using SBRW.Launcher.Core.Extras.Proxy_.Log_;
using System;
using System.Linq;
...
namespace SBRW.Launcher.Core.Extras.Proxy_.Nancy_
{
    internal class Proxy_Request : IApplicationStartup
    {
        ...
        private TextResponse OnError(NancyContext context, Exception Error)
        {
            ...
            Communication_Nancy.RecordEntry(Proxy_Server.Instance.GetServerName(), "LAUNCHER", CommunicationLogEntryType.Error,
        ...
        private async Task<Response> ProxyRequest(NancyContext Local_Context, CancellationToken cancellationToken)
        {
            ...
            if (!path.StartsWith("/nfsw/Engine.svc"))
            {
                ...
                return "Soapbox Race World: Launcher Core Extras Version " + AssemblyName.GetAssemblyName("SBRW.Launcher.Core.Extras.dll").Version.ToString() ?? string.Empty;
            }
            else
            {
                path = path.Substring("/nfsw/Engine.svc".Length);

                UrlFlurl resolvedUrl = new UrlFlurl(Proxy_Server.Instance.GetServerUrl()).AppendPathSegment(path, false);
                ...
                Communication_Nancy.RecordEntry(Proxy_Server.Instance.GetServerName(), "SERVER", CommunicationLogEntryType.Request,
                ...
                Communication_Nancy.RecordEntry(Proxy_Server.Instance.GetServerName(), "SERVER", CommunicationLogEntryType.Response,
